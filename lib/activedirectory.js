...
const escapeLdapDn = (dn) => {
  console.log('value for escape >>>>', dn);
...
function defaultClearDn(val) {
  return escapeLdapDn(val)
}

var clearDn = defaultClearDn;
...
var ActiveDirectory = function(url, baseDN, username, password, defaults) {
  if (this instanceof ActiveDirectory) {
    ...
    if (typeof(url) === 'string') {
      ...
      if (this.opts.clearDn && typeof this.opts.clearDn === 'function') {
        clearDn = this.opts.clearDn;
      }
...
function search(baseDN, opts, callback) {
  ...
  console.log('prepared request params >>>>>', JSON.stringify({baseDN, opts: getLdapOpts(opts), controls}));
  client.search(baseDN, getLdapOpts(opts), controls, function onSearch(err, res) {
    ...
    res.on('end', function(result) {
      ...
      console.log('got response with params >>>>>', JSON.stringify({baseDN, opts: getLdapOpts(opts), controls}));
      onSearchEnd(result);
    });
  });
}
